...
class CNNPathActor(nn.Module):
    def __init__(self, input_channels=1, action_size=1, hidden_size=128, std_scale=0.5, device=None):
        super().__init__()
    ...
    def forward(self, x, hidden=None):
        ...
        log_std = torch.clamp(self.std_head(x), min=-2, max=2) 
        std = (F.softplus(log_std) + 1e-3) * self.std_scale

        dist = torch.distributions.Normal(mu, std)
        return dist, None  # pas de hidden state
